    ...
    <div className='w-full z-500 h-[80px] flex items-center justify-between py-3 wrapper shadow-[0_0px_0_1px_rgba(0,0,0,0.25)]'>
      ...
      <div>
        <ul className='flex items-center justify-between gap-25 poppins'>
          ...
          {
            !isLogin ? (<li className='flex items-center gap-5 cursor-pointer'>
              <Link to={"./login"}>
          ...
          }
          {
            ...
            <div onClick={()=>{
              ...
              />
                ...
                <div className={` ${isOpen ? 'block' : 'hidden'} absolute flex flex-col gap-4 mt-4 cursor-pointer right-0 whitespace-nowrap bg-white text-black border-1 border-[#479773] p-2`}>
                <div className="hover:text-[#479773]"><Link to={`/userdetail/${isLogin?.uid}`}>Your Profile</Link></div>
                <button className="p-2 w-full bg-red-500  transition-shadow duration-300 cursor-pointer text-white rounded-sm" onClick={logOut}>Log Out</button>
               </div>
          </li>
            ) : ''
          }
